...
1 : mise en veille et reveil par interruption ou RTC : 0


version mise en veille com avec liaison série pour map : 0)
gestion du reveil par interruption RTC : 1/2

but :  usage version simple du capteur avec emission radio, 
...
void loop() {
    ...
    //set_sleep_mode(SLEEP_MODE_PWR_DOWN);//choix du mode sleep
    //attachInterrupt(SQW_PIN, alarmIsr, FALLING);//parametrage interruption RTC
    //sleep_enable();//autorisation de mise en sommeil
    //sleep_mode();//mise en veille
    // first thing after waking from sleep:     
